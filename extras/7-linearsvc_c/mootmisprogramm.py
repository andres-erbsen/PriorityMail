...
#~for i in range(len(messages_as_text)):
#~    if isreplied[i]:
#~        messages_as_text[i] += '\nsee_kiri_on_vastatud'
...
classificationmethods = [
        ('Counts-linearSVC',removeshortwords(3),CountVectorizer(),none, SVC(kernel='linear',C=0.5,probability=True)),
        ('TFIDF-linearSVC', removeshortwords(3),CountVectorizer(),tfidf,SVC(kernel='linear',C=2,probability=True)),
        #~ ('TF-linearSVC-textonly',    textonly,CountVectorizer(),tf,   SVC(kernel='linear',C=0.5,probability=True)),
        #~ ('TF-rbfSVC-textonly',   textonly, CountVectorizer(),tf,   SVC(kernel='rbf',C=4,gamma=1,probability=True)),
        ('TF-linearSVC',    none,CountVectorizer(),tf,   SVC(kernel='linear',C=0.5,probability=True)),
        #~ ('TF-rbfSVC',    none,CountVectorizer(),tf,   SVC(kernel='rbf',C=4,gamma=1,probability=True)),
        #~ ('TF-sigmoidSVC',    removeshortwords(3),CountVectorizer(),tf,   SVC(kernel='sigmoid',C=0.5,probability=True)),
        #~ ('TF-polySVC',    removeshortwords(3),CountVectorizer(),tf,   SVC(kernel='poly',C=2^7,degree=2,probability=True)),
        #~ ('Counts-MNB',removeshortwords(3),CountVectorizer(),none,MultinomialNB(alpha=0.001)),
        #~ ('TF-MNB-textonly',textonly,CountVectorizer(),tf,MultinomialNB(alpha=0.001)),
        ('TF-MNB alpha=0.001',none,CountVectorizer(),tf,MultinomialNB(alpha=0.001)),
        ]

classificationmethods = [('TF-linearSVC, C=2^%d'%i,    removeshortwords(3),CountVectorizer(),tf,   SVC(kernel='linear',C=2.0**i,probability=True)) for i in range(-12,12,2)]
#~ classificationmethods = [('TFIDF-linearSVC, C=2^%d'%i,    removeshortwords(3),CountVectorizer(),tfidf,   SVC(kernel='linear',C=2.0**i,probability=True)) for i in range(-2,12,1)]
#~ classificationmethods =  [('TF-rbfSVC, C=2^%d, gamma=2^%d'%(i,j),    removeshortwords(3),CountVectorizer(),tf,   SVC(kernel='rbf',C=2.0**i,gamma=2.0**j,probability=True)) for i in range(-5,4,1) for j in [0]]
#~ classificationmethods =  [('TF-polySVC, C=2^%d, degree=%d'%(i,j),    removeshortwords(3),CountVectorizer(),tf,   SVC(kernel='poly',C=2.0**i,degree=j,probability=True)) for i in range(5,19,2) for j in [2]]
...
for name,prevect,vect,postvect,clf in classificationmethods:
    for k, (train, test) in enumerate(cv):
        ...
        gotprobabs = True
    try:
        names = vect.vocabulary.keys()
        try:
            weights = clf.coef_.todense().tolist()[0]
        except:
            weights = clf.coef_.tolist()[0]
        weightedfeatures = zip(names,weights)
        significant = heapq.nlargest(50,weightedfeatures,key=lambda (word,weight): abs(weight))
        significant.sort(key=lambda (word,weight): weight)
        print '%s: %d most significant features:' % (name,len(significant))
        for word,weight in significant:
            print '   ',word[:30],' '*(30-len(word)),weight
        print
