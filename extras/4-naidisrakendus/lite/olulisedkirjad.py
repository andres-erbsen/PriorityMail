...
def main():
    ...
    if action == 'train':
        ...
        for (importance,message) in importantmessages:
            headline = u''
            # headline += ' '.join(getheaders(message,'message-id'))
            # headline += " | "
            ...
            # headline = '-'.join(map(str,map(ord,sha512(headline.encode('unicode-escape')).digest()))[:5]) + ' | ' # Anonymizes messages.
            headline += "%0.3g" % importance
            print headline.encode('ascii',errors='ignore')
